...
class OutputToScreen(Output):
    def __init__(self, subject, *, display_data: bool, stats_interval: int = 0):
        ...
        :param display_data: Boolean specifying the output of captured
            data.
        :param stats_interval: Interval (in seconds) for displaying live
            packet capture statistics. Set to 0 to disable statistics.
        """
    ...
    def update(self, frame) -> None:
        ...
        if self.stats_interval > 0 and time.time() - self.start_time >= self.stats_interval:
    ...
    def _display_statistics(self) -> None:
        ...
        print("\n[Statistics]")
        print(f"{i}Total Packets: {self.packet_count}")
        print(f"{i}Time Elapsed: {elapsed_time:.2f} seconds")
        print(f"{i}Packets per Second: {self.packet_count / elapsed_time:.2f}\n")
